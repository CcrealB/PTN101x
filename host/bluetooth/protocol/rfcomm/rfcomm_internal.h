...
extern int32_t rfcomm_ack_timeout;
extern int32_t rfcomm_mcc_timeout;
...
struct rfcomm_dlc {
    ...
    int32_t                rd_txcred;  /* transmit credits (unused) */
    int32_t                rd_pending; /* packets sent but not complete */
...
result_t rfcomm_session_send_frame(rfcomm_session_t *, int32_t, uint8_t);
result_t rfcomm_session_send_uih(rfcomm_session_t *, struct rfcomm_dlc *,
    int32_t, struct mbuf *);
result_t rfcomm_session_send_mcc(rfcomm_session_t *, int32_t, uint8_t,
    void *, int32_t);
void rfcomm_session_timeout(void *);

#ifdef J_DEBUG
void rfcomm_dlc_dump(const char *file, int32_t line, struct rfcomm_dlc *dlc);
void rfcomm_session_dump(const char *file, int32_t line,
    struct rfcomm_session *rs, uint8_t all);
void rfcomm_active_list_dump(const char *file, int32_t line, uint8_t all);
