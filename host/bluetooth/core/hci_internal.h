...
typedef struct hci_link {
    ...
    int32_t                        hl_txqlen;     /* number of fragments */
...
result_t hci_acl_send_auth_req(hci_link_t *link,uint8_t req);
...
BOOL unit_feature_supported(hci_unit_t * hci_unit, uint8_t octet,
    uint8_t bit);
