...
static const uint8_t SBC_ENCODER_FRAME_ID[] =
{
    0x08, 0x38, 0xC8, 0xF8
};
...
static inline void _sbc_encoder_analyze_eight(const int32_t *in, int32_t *out)
{
    ...
    t0 += SBC_ENCODER_PROTO_8[36] * (in[8]  + in[72]);
    t0 += SBC_ENCODER_PROTO_8[37] * (in[24] + in[56]);
    t0 += SBC_ENCODER_PROTO_8[38] * in[40];
    t0  = SCALE8_STAGE1(t0);

    t1  = SBC_ENCODER_PROTO_8[11] * in[1];
    t1 += SBC_ENCODER_PROTO_8[12] * in[17];
    t1 += SBC_ENCODER_PROTO_8[13] * in[33];
    t1 += SBC_ENCODER_PROTO_8[14] * in[49];
    t1 += SBC_ENCODER_PROTO_8[15] * in[65];
    t1 += SBC_ENCODER_PROTO_8[31] * in[7];
    t1 += SBC_ENCODER_PROTO_8[32] * in[23];
    t1 += SBC_ENCODER_PROTO_8[33] * in[39];
    t1 += SBC_ENCODER_PROTO_8[34] * in[55];
    t1 += SBC_ENCODER_PROTO_8[35] * in[71];
    t1  = SCALE8_STAGE1(t1);

    t2  = SBC_ENCODER_PROTO_8[6 ] * in[2];
    t2 += SBC_ENCODER_PROTO_8[7 ] * in[18];
    t2 += SBC_ENCODER_PROTO_8[8 ] * in[34];
    t2 += SBC_ENCODER_PROTO_8[9 ] * in[50];
    t2 += SBC_ENCODER_PROTO_8[10] * in[66];
    t2 += SBC_ENCODER_PROTO_8[26] * in[6];
    t2 += SBC_ENCODER_PROTO_8[27] * in[22];
    t2 += SBC_ENCODER_PROTO_8[28] * in[38];
    t2 += SBC_ENCODER_PROTO_8[29] * in[54];
    t2 += SBC_ENCODER_PROTO_8[30] * in[70];
    t2  = SCALE8_STAGE1(t2);

    t3  =  SBC_ENCODER_PROTO_8[16] * in[3];
    t3 +=  SBC_ENCODER_PROTO_8[17] * in[19];
    t3 +=  SBC_ENCODER_PROTO_8[18] * in[35];
    t3 +=  SBC_ENCODER_PROTO_8[19] * in[51];
    t3 +=  SBC_ENCODER_PROTO_8[20] * in[67];
    ...
    t4  =  SBC_ENCODER_PROTO_8[2 ] * in[4];
    t4 +=  SBC_ENCODER_PROTO_8[3 ] * in[20];
    t4 +=  SBC_ENCODER_PROTO_8[4 ] * in[36];
    t4 +=  SBC_ENCODER_PROTO_8[5 ] * in[52];
    t4 +=  SBC_ENCODER_PROTO_8[39] * in[68];
    t4  =  SCALE8_STAGE1(t4);

    t5  =  SBC_ENCODER_PROTO_8[35] * in[9];
    t5 +=  SBC_ENCODER_PROTO_8[34] * in[25];
    t5 +=  SBC_ENCODER_PROTO_8[33] * in[41];
    t5 +=  SBC_ENCODER_PROTO_8[32] * in[57];
    t5 +=  SBC_ENCODER_PROTO_8[31] * in[73];
    t5 += -SBC_ENCODER_PROTO_8[15] * in[15];
    t5 += -SBC_ENCODER_PROTO_8[14] * in[31];
    t5 += -SBC_ENCODER_PROTO_8[13] * in[47];
    t5 += -SBC_ENCODER_PROTO_8[12] * in[63];
    t5 += -SBC_ENCODER_PROTO_8[11] * in[79];
    t5  =  SCALE8_STAGE1(t5);

    t6  =  SBC_ENCODER_PROTO_8[30] * in[10];
    t6 +=  SBC_ENCODER_PROTO_8[29] * in[26];
    t6 +=  SBC_ENCODER_PROTO_8[28] * in[42];
    t6 +=  SBC_ENCODER_PROTO_8[27] * in[58];
    t6 +=  SBC_ENCODER_PROTO_8[26] * in[74];
    t6 += -SBC_ENCODER_PROTO_8[10] * in[14];
    t6 += -SBC_ENCODER_PROTO_8[9 ] * in[30];
    t6 += -SBC_ENCODER_PROTO_8[8 ] * in[46];
    t6 += -SBC_ENCODER_PROTO_8[7 ] * in[62];
    t6 += -SBC_ENCODER_PROTO_8[6 ] * in[78];
    t6  =  SCALE8_STAGE1(t6);
    ...
    t7 += -SBC_ENCODER_PROTO_8[20] * in[13];
    t7 += -SBC_ENCODER_PROTO_8[19] * in[29];
    t7 += -SBC_ENCODER_PROTO_8[18] * in[45];
    t7 += -SBC_ENCODER_PROTO_8[17] * in[61];
    t7 += -SBC_ENCODER_PROTO_8[16] * in[77];
    t7  =  SCALE8_STAGE1(t7);

    res  =  SBC_ENCODER_ANAMATRIX_8[6] * t0;
    res +=  SBC_ENCODER_ANAMATRIX_8[3] * t1;
    res +=  SBC_ENCODER_ANAMATRIX_8[0] * t2;
    res +=  SBC_ENCODER_ANAMATRIX_8[2] * t3;
    res +=  SBC_ENCODER_ANAMATRIX_8[7] * t4;
    res +=  SBC_ENCODER_ANAMATRIX_8[4] * t5;
    res +=  SBC_ENCODER_ANAMATRIX_8[1] * t6;
    res +=  SBC_ENCODER_ANAMATRIX_8[5] * t7;
    *out++ = SCALE8_STAGE2(res);

    res  = -SBC_ENCODER_ANAMATRIX_8[6] * t0;
    res += -SBC_ENCODER_ANAMATRIX_8[5] * t1;
    res +=  SBC_ENCODER_ANAMATRIX_8[1] * t2;
    res +=  SBC_ENCODER_ANAMATRIX_8[3] * t3;
    res +=  SBC_ENCODER_ANAMATRIX_8[7] * t4;
    res += -SBC_ENCODER_ANAMATRIX_8[2] * t5;
    res += -SBC_ENCODER_ANAMATRIX_8[0] * t6;
    res += -SBC_ENCODER_ANAMATRIX_8[4] * t7;
    *out++ = SCALE8_STAGE2(res);

    res  = -SBC_ENCODER_ANAMATRIX_8[6] * t0;
    res += -SBC_ENCODER_ANAMATRIX_8[2] * t1;
    res += -SBC_ENCODER_ANAMATRIX_8[1] * t2;
    res +=  SBC_ENCODER_ANAMATRIX_8[4] * t3;
    res +=  SBC_ENCODER_ANAMATRIX_8[7] * t4;
    res +=  SBC_ENCODER_ANAMATRIX_8[5] * t5;
    res +=  SBC_ENCODER_ANAMATRIX_8[0] * t6;
    res +=  SBC_ENCODER_ANAMATRIX_8[3] * t7;
    *out++ = SCALE8_STAGE2(res);

    res  =  SBC_ENCODER_ANAMATRIX_8[6] * t0;
    res += -SBC_ENCODER_ANAMATRIX_8[4] * t1;
    res += -SBC_ENCODER_ANAMATRIX_8[0] * t2;
    res +=  SBC_ENCODER_ANAMATRIX_8[5] * t3;
    res +=  SBC_ENCODER_ANAMATRIX_8[7] * t4;
    res +=  SBC_ENCODER_ANAMATRIX_8[3] * t5;
    res += -SBC_ENCODER_ANAMATRIX_8[1] * t6;
    res += -SBC_ENCODER_ANAMATRIX_8[2] * t7;
    *out++ = SCALE8_STAGE2(res);
...
int32_t sbc_encoder_ctrl(SbcEncoderContext* sbc, uint32_t cmd, uint32_t arg)
{
    ...
    case SBC_ENCODER_CTRL_CMD_SET_ALLOCATION_METHOD:
        if(arg > 1)
        {
            return SBC_ENCODER_ERROR_INVALID_CTRL_ARG;
        }
    ...
    case SBC_ENCODER_CTRL_CMD_GET_SUBBAND_MODE:
        *(uint32_t*)arg = sbc->header.subband_mode;
        break;
    default:
        return SBC_ENCODER_ERROR_INVALID_CTRL_CMD;
    }
...
int32_t sbc_encoder_encode(SbcEncoderContext* sbc, const int16_t* pcm)
{
    ...
    sbc->frame_id[0]  = 0x01;
    sbc->frame_id[1]  = SBC_ENCODER_FRAME_ID[sbc->frame_index++];
