...
typedef struct _MP3Id3 {
	unsigned char header[3];			//TAG
	unsigned char title[30];			//歌曲名
	unsigned char artist[30];			//歌手名
	unsigned char album[30];			//专辑名
	unsigned char year[4];		 		//日期
	unsigned char comment[28];			//注释内容
	unsigned char reserved;				//保留
	unsigned char track;				//音轨
