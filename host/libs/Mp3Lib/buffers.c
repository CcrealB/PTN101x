...
void ClearMP3(MP3DecInfo *mp3DecInfo)
{
    ...
    //++ by Borg @ 230413 :clear sb_samples and synth_buf pointer, must reinit after ClearMP3
    L2DecodeContext* l2i = (L2DecodeContext *)(mp3DecInfo->L2DecInfo);
...
#define MALLOC_DEBUG
    ...
    if (!fh || !si || !sfi || !hi || !di || !mi || !sbi || !l2i) {
        ...
        FreeBuffers(mp3DecInfo_pointer);	/* ��ȫ�ص��� - ֻ�ͷ��ڴ棬�ѳɹ����� */
        return 0;
    }
...
void FreeBuffers(MP3DecInfo *mp3DecInfo)
{
	...
	if(mp3DecInfo->FrameHeaderPS)
	{
    	jfree(mp3DecInfo->FrameHeaderPS);
        mp3DecInfo->FrameHeaderPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_0\r\n");
    }
    
    if(mp3DecInfo->SideInfoPS)
	{
    	jfree(mp3DecInfo->SideInfoPS);
        mp3DecInfo->SideInfoPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_1\r\n");
    }
	
    if(mp3DecInfo->ScaleFactorInfoPS)
	{
    	jfree(mp3DecInfo->ScaleFactorInfoPS);
        mp3DecInfo->ScaleFactorInfoPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_2\r\n");
    }
	
    if(mp3DecInfo->HuffmanInfoPS)
	{
    	jfree(mp3DecInfo->HuffmanInfoPS);
        mp3DecInfo->HuffmanInfoPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_3\r\n");
    }
	
    if(mp3DecInfo->DequantInfoPS)
	{
    	jfree(mp3DecInfo->DequantInfoPS);
        mp3DecInfo->DequantInfoPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_4\r\n");
    }
	...
	l2i->inbuf_ptr = NULL;
	l2i->sb_samples = NULL;
	if(mp3DecInfo->L2DecInfo)
	{
    	jfree(mp3DecInfo->L2DecInfo);
        mp3DecInfo->L2DecInfo = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_5\r\n");
    }
	
    if(mp3DecInfo->IMDCTInfoPS)
	{
    	jfree(mp3DecInfo->IMDCTInfoPS);
        mp3DecInfo->IMDCTInfoPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_6\r\n");
    }
	
    if(mp3DecInfo->SubbandInfoPS)
	{
    	jfree(mp3DecInfo->SubbandInfoPS);
        mp3DecInfo->SubbandInfoPS = NULL;
    }
    else
    {
        os_printf("         err_FreeBuffers_7\r\n");
    }
