...
{
    uint8_t* address;     /**< memory buff address      */
    uint32_t addr_base;   /**< memory base address      */
    uint32_t capacity;    /**< memory capacity in bytes */
    volatile uint32_t wp;          /**< write point in bytes     */
    volatile uint32_t rp;          /**< read point in bytes      */
    void*    dma;
    uint32_t dma_type;
}RingBufferContext;
...
/** @brief ring buffer reset rp equal to wp, by read side */
void ring_buffer_reset_rp(RingBufferContext* rb);

/** @brief ring buffer reset wp equal to rp, by write side */
