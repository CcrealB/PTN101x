...
#if defined(KT56) && defined(MZ_200K) || defined(ZT_M184)
...
uint32_t KtFreq[40]={
		650600, 652100, 653600, 655100, 656600,
		658100, 659100, 660100, 661100, 662100,
		663100, 664100, 665100, 666100, 667100,
		668100, 669100, 670100, 671100, 672100,

		679600, 681100, 682600, 684100, 685600,
		687100, 688100, 689100, 690100, 691100,
		692100, 693100, 694100, 695100, 696100,
		697100, 698100, 699100, 700100, 701100,
};
...
uint8_t freqSearch1(uint8_t chip)
{
	...
	KT_WirelessMicRx_Volume(0);	  //关闭音量
//	printf("============================================\n");
	for(WCh=SCh; WCh<BCh; WCh++){
		KT_WirelessMicRx_FastTune(KtFreq[WCh]);
...
//			KT56_DBG_INFO("==== chip: %d   Max Freq: %ld   rssi: %d\n",(uint16_t)chipSel, rssiMaxFreq, (uint16_t)rssiMax);
		...
		//		SysInf.BchPair = KT_Bus_Read(0x024A,chipSel)<<8 | KT_Bus_Read(0x024B,chipSel);
				KT56_DBG_INFO("==== Search ChB rssi:%d  Ch: %d  Pair: %04X\n",rssi, WCh, SysInf.BchPair);
		//		SysInf.PAIR |= 0x81;
			//	Send_Inf(0x87,((rssiMaxFreq- Freq_Top)/Freq_Step)+1);
			}else{
				SysInf.MicCh_A = WCh;
		//		SysInf.AchPair =  KT_Bus_Read(0x024A,chipSel)<<8 | KT_Bus_Read(0x024B,chipSel);
				KT56_DBG_INFO("==== Search ChA rssi:%d  Ch: %d  Pair: %04X\n",rssi, WCh, SysInf.AchPair);
		//		SysInf.PAIR |= 0x41;
...
//				DISPLAY_BATT((CHA_DataR[0]-1000)/230, 1);
				KT56_DBG_INFO("CHA == R00:%04X \n",CHA_Data[0]);
			}
			if(CHA_DataR[2]!=CHA_Data[2]){
				CHA_DataR[2]=CHA_Data[2];
				EF_Mode = (CHA_Data[2]>>8)-0xE0;
				KT56_DBG_INFO("CHA == R02:%04X \n",CHA_Data[2]);
			}
			if(CHA_DataR[3]!=CHA_Data[3]){
				CHA_DataR[3]=CHA_Data[3];
				KT56_DBG_INFO("CHA == R03:%04X \n",CHA_Data[3]);
			}
#if 1
			if(CHA_DataR[1] != CHA_Data[1]){
				KT56_DBG_INFO("CHA == R01:%04X  %04X\n",CHA_Data[1],CHA_DataR[1]);
				CHA_DataR[1] = CHA_Data[1];
				if(SysInf.PAIR&0x40){
					SysInf.PAIR &= (~0x40);
...
//					Send_Inf(0x45,SysInf.MicCh_A+1);
					KT56_DBG_INFO("==== ChA Pair Write...\n");
				}else{
					if(SysInf.MicCh_A != (CHA_Data[1]&0xFF)-1){
						SysInf.MicCh_A = (CHA_Data[1]&0xFF)-1;
						SysInf.AchPair = CHA_Data[1];
						GroupWrite(0, 2);
						Set_CH(1);
						KT56_DBG_INFO("==== ChA CH Write...\n");
					}
				}
...
//				DISPLAY_BATT((CHB_DataR[0]-1000)/230, 2);
				KT56_DBG_INFO("CHB == R00:%04X \n",CHB_Data[0]);
			}
			if(CHB_DataR[2]!=CHB_Data[2]){
				CHB_DataR[2]=CHB_Data[2];
				EF_Mode = (CHB_Data[2]>>8)-0xE0;
				KT56_DBG_INFO("CHB == R02:%04X \n",CHB_Data[2]);
			}
			if(CHB_DataR[3]!=CHB_Data[3]){
				CHB_DataR[3]=CHB_Data[3];
				KT56_DBG_INFO("CHB == R03:%04X \n",CHB_Data[3]);
			}
#if 1
...
//					Send_Inf(0x46,SysInf.MicCh_B+1);
					...
					if(SysInf.MicCh_B != (CHB_Data[1]&0xFF)+19){
						SysInf.MicCh_B = (CHB_Data[1]&0xFF)+19;
						SysInf.BchPair = CHB_Data[1];
						GroupWrite(0, 2);
						Set_CH(2);
						KT56_DBG_INFO("==== ChB CH Write...\n");
					}
				}
