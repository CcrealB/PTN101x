...
__INLINE__ t_deviceIndex DL_Get_Device_Index_By_Am_Addr(uint32_t am_addr)
{
    t_devicelink* p_device_link = (t_devicelink*) SYSconfig_Get_Device_Links_Location();
 ...
 *
 ********************************************************************/
__INLINE__ void DL_Set_Role_Peer(t_devicelink* pDevLink, uint32_t role_of_peer)
{
...
__INLINE__ uint8_t DL_Get_Piconet_Index(const t_devicelink* p_device_link)
{
...
{
    extern uint8_t DL_piconet_index_pool;
 ...
 */
__INLINE__ uint32_t DL_Get_Remote_Rxbuf_Full(const t_devicelink* pDevLink)
{
    return (pDevLink->remote_rx_buffer_full);
}

__INLINE__ void DL_Set_Remote_Rxbuf_Full(t_devicelink* pDevLink, uint32_t status)
{
    pDevLink->remote_rx_buffer_full = status;
}

__INLINE__ void DL_Set_Local_Rxbuf_Full(t_devicelink* pDevLink, uint32_t status)
{
    pDevLink->local_rx_buffer_full = status;
}

__INLINE__ uint32_t DL_Get_Local_Rxbuf_Full(const t_devicelink* pDevLink)
{
    return (pDevLink->local_rx_buffer_full);
}


__INLINE__ void DL_Set_Rx_Expected_Broadcast_Seqn(t_devicelink* pDevLink, uint32_t seqn)
{
...
__INLINE__ uint32_t DL_Get_Rx_Expected_Broadcast_Seqn(const t_devicelink* pDevLink)
{
 ...
 *
 */
__INLINE__ uint32_t DL_Get_Tx_Ack_Pending(const t_devicelink* pDevLink)
{
 ...
 *
 */
__INLINE__ void DL_Set_Tx_Ack_Pending(t_devicelink* pDevLink, uint32_t ack_pending)
{
#if (PRH_BS_CFG_SYS_LMP_EXTENDED_SCO_SUPPORTED == 1)
...
__INLINE__ uint8_t DL_Get_Power_Level(const t_devicelink* pDevLink)
{
    return pDevLink->power_level;
}

__INLINE__ void DL_Set_Power_Level(t_devicelink* pDevLink, uint8_t power_units)
{
...
{
	extern BOOL epc_enabled /* global TCI override */;
...
__INLINE__ void DL_Set_Local_Slot_Offset(t_devicelink* p_device_link, uint16_t offset)
{
    p_device_link->local_slot_offset_us = offset;
}

__INLINE__ uint16_t DL_Get_Peer_Slot_Offset(const t_devicelink* p_device_link)
{
...
__INLINE__ void
    DL_Set_Piconet_Clock_Last_Access(t_devicelink* p_device_link, uint32_t bt_clock)
{
    p_device_link->piconet_clock_last_access = bt_clock;

}

__INLINE__ uint8_t  DL_Get_Clock_Jitter(const t_devicelink* p_device_link)
{
...
__INLINE__ uint32_t DL_Is_Local_Buffer_Full(const t_devicelink* pDevLink)
{
/*
...
{
    pDevLink->packet_type = (uint8_t) type;
}
...
__INLINE__ uint8_t* DL_Get_Tx_Qpdu(const t_devicelink* pDevLink)
{
...
__INLINE__ void DL_Set_Tx_Qpdu(t_devicelink* pDevLink, uint8_t* buffer)
{
    assert(pDevLink != 0);
    assert(pDevLink->tx_descriptor != 0);
...
{
    assert(pDevLink != 0);
    pDevLink->local_tx_L2CAP_flow = (uint8_t) flow;
}
...
{
    assert(pDevLink != 0);
    pDevLink->local_rx_L2CAP_flow = (uint8_t) flow;
}
...
__INLINE__ void DL_Set_Active_SCO_Index(t_devicelink* p_device_link, uint8_t sco_index)
{
...
__INLINE__ void
    DL_Set_Encryption_Key_Ref(t_devicelink* pDevLink, uint8_t *p_encryption_key)
{
 ...
 */
__INLINE__ void
    DL_Set_Encryption_Key_Length(t_devicelink* pDevLink, uint8_t encryption_key_length)
{
    pDevLink->hw_encryption_key_len = _SYS_GET_CHAR_8_BITS(encryption_key_length - 1);
}
__INLINE__ uint8_t DL_Get_Encryption_Key_Length(const t_devicelink* pDevLink)
{
 ...
 * DL_Set_PTT_ACL
 */
__INLINE__ void DL_Set_PTT_ACL(t_devicelink* pDevLink, BOOL ptt_acl)
{
 ...
 * DL_Set_PTT_ESCO
 */
__INLINE__ void DL_Set_PTT_ESCO(t_devicelink* pDevLink, BOOL ptt_esco)
{
