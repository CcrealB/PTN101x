...
t_error USLCinq_Inquiry_Request(t_lap inq_lap,
                        t_slots inquiry_timeout, uint16_t Ninquiry);
void USLCinq_Inquiry_Cancel(void);
void USLCinq_Inquiry(BOOL look_ahead_to_next_frame);
...
uint8_t* USLCinq_Get_Stored_Extended_Inquiry_Response(void);
BOOL USLCinq_Is_Extended_Inquiry_Response_Frame(void);
#endif
uint8_t USLCinq_Is_Inquiry_Suspended(void);
uint8_t USLCinq_Get_Inq_State(void);
#endif
