...
typedef struct s_tm_control 
{
     ...
     */
    uint8_t         transmitter_test;
    uint8_t         loopback_test;

#ifdef USLC_TEST_MODE_EXTENSIONS
    /*
     * Testmode extensions
     */
    uint8_t         cycle_length_test;
    uint8_t         transmitter_test_all_packets;
    t_length       next_cycle_length;
    uint8_t         send_packet_every_poll_period;
     ...
     */
    uint8_t         original_data_whitening_state;
...
void USLCtm_Build_Test_Packet_Payload(t_testScenario testScenario, uint8_t *txBuf, t_length packetLength);
