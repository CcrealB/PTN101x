...
t_error USLCsp_Request(t_deviceIndex device_index, BOOL sp_override);
t_error USLCsp_Cancel(void);
uint8_t USLCsp_State_Dispatcher(BOOL);
...
void USLCsp_Set_eSCO_Next_Time(uint32_t next_esco_clk_time );
...
uint32_t USLCsp_Get_eSCO_Next_Time(void);
