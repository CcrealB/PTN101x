...
BOOL USLCchac_check_slave_activity_start_next_frame(void);
BOOL USLCchac_is_next_activity_scan_activation(void);

BOOL USLCchac_Is_Inquiry_Scan_Rand_Wait(void);
...
RAM_CODE void USLCchac_Set_Sleep_Request(BOOL sleep_request);
RAM_CODE BOOL USLCchac_Get_Sleep_Request(void);
RAM_CODE t_state USLCchac_get_device_state(void);

t_error USLCchac_Procedure_Request(t_ulc_procedure procedure, BOOL status);

void USLCchac_Procedure_Start(t_ulc_procedure procedure, t_devicelink *p_device_link);

DRAM_CODE void USLCchac_Procedure_Finish(BOOL set_previous_super_state);
...
BOOL USLCchac_Is_Sleep_Possible(t_clock *next_instant);

BOOL USLCchac_Is_Sleep_Request_Pending(void);

BOOL USLCchac_Is_R2P_Request_Pending(void);
RAM_CODE void USLCchac_Force_To_Tx_State_If_Req(void);
#if (PRH_BS_CFG_SYS_ESCO_IN_SCATTERNET==1)
void USLCchac_Check_Switch_Piconet_IRQ_Context(uint8_t irq);
#endif
#endif
