...
#define is_EDR_BR_packet(pkt)  (((1 <<  (pkt)) &  BR_PACKET_MASK) !=0)
...
extern const uint8_t LSLCacc_packet_slots[];
...
void LSLCacc_Sync_Setup_SCO_ROUTE_Ex(t_sco_fifo sco_fifo, BOOL sco_route);
__INLINE__ BOOL LSLCacc_Get_SCO_ROUTE(t_sco_fifo sco_fifo);
#endif

DRAM_CODE BOOL LSLCacc_Is_SCO_CFG_Via_ACL_Buffer(t_sco_fifo sco_fifo);
DRAM_CODE BOOL LSLCacc_Is_SCO_CFG_Eight_Bit_Input_Mode(t_sco_fifo sco_fifo);
DRAM_CODE void LSLCacc_Prepare_Rx_Non_Signal_Connection(const t_devicelink *p_dev_link);

DRAM_CODE void LSLCacc_Prepare_Rx_ID(t_state context);
DRAM_CODE void LSLCacc_Prepare_Rx_FHS(t_state context, uint32_t crc_hec_init);
...
RAM_CODE void LSLCacc_Prepare_Rx_Testmode_R2P(const t_devicelink *p_dev_link);
void LSLCacc_Prepare_Tx_ID(t_state context);
RAM_CODE void LSLCacc_Prepare_Tx_Rx_FHS(t_state context, uint32_t crc_hec_init);
RAM_CODE void LSLCacc_Prepare_Tx_Rx_Connection(const t_devicelink *p_dev_link,
                                      uint32_t crc_hec_init,
                                      t_encrypt_mode encrypt_mode,
                                      t_state context);

void LSLCacc_Prepare_Tx_Rx_Encryption(
     const uint8_t *p_encryption_key, uint8_t encryption_key_length);
...
void LSLCacc_Extract_EIR_Payload(uint8_t *p_eir_packet, uint8_t rx_length);

void LSLCacc_Initialise(void);

#if (LC_DEFERRED_PAYLOAD_EXTRACTION_SUPPORTED==1)
void LSLCacc_Enable_Deferred_Extraction(BOOL status);
...
RAM_CODE void LSLCacc_Attempt_RSSI_Read(BOOL);
