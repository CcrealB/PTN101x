 ...
 *                      uint8_t used for optimum packing
 */
enum
{
...
typedef struct s_hop_selection_kernel
{
   ...
   uint8_t        Ai;                    /* A extracted from uap_lap only */
   uint8_t        Bi;                    /* B extracted from uap_lap only */
   uint8_t        Ci;                    /* C extracted from uap_lap only */
   uint8_t        EFi;                   /* E extracted from uap_lap only */
   uint16_t       Di;                    /* D extracted from uap_lap only */

   uint32_t       sum_before_mod;        /* Current sum before mod       */
} t_hs_kernel;
...
typedef struct s_hop_selection_engine
{
   ...
   uint8_t        frozen_X_inquiry_scan; /* Implicitly set here          */
   uint8_t        phase_N_inquiry_scan;  /* Implicitly set here          */
   uint8_t        frozen_X_page_scan;    /* Implicitly set here          */
   uint8_t        frozen_X_page;         /* Explicitly set by USLC_Page  */
   uint8_t        X;                     /* Current X:                   */
...
DRAM_CODE uint32_t   LSLChop_Get_X(void);
...
DRAM_CODE uint32_t LSLChop_Execute_79_Hop_Kernel_Connection_Summation(
    t_clock clk, t_hs_kernel *p_hs_kernel);
uint8_t LSLChop_Get_AFH_k_prime(t_clock clk, t_hs_kernel *pKernel);
