...
typedef struct s_afh_kernel
{
     ...
     */
    uint8_t  afh_current_channel_map[AFH_BYTES_PER_CHANNEL_MAP];
    uint8_t  afh_current_num_used_chns;
    uint8_t  afh_current_master_tx_freq;

    /*
     * AFH_instant[C0028] 4 uint32_t slots Bits 27:1 of the Bluetooth
     * master clock value at the time of switching hop sequences
     */
    uint8_t  afh_next_channel_map[AFH_BYTES_PER_CHANNEL_MAP];
    uint8_t  afh_next_num_used_chns;
     ...
     */
    uint8_t  afh_mapping_table_1[AFH_N_MAX_USED_CHANNELS];
    uint8_t  afh_mapping_table_2[AFH_N_MAX_USED_CHANNELS];

    uint8_t  *p_current_mapping_table;
    uint8_t  *p_next_mapping_table;
} t_afh_kernel;
...
typedef struct s_afh_device_link
{
    ...
    uint8_t  afh_current_mode;
    uint8_t  afh_next_mode;
} t_afh_link;

#if PRH_BS_CFG_SYS_LMP_AFH_CAPABLE_SUPPORTED

void LSLCafh_Initialise(void);
void LSLCafh_Assign_AFH_Kernel(uint8_t piconet_index,
...
const uint8_t* LSLCafh_LM_Read_AFH_Channel_Map(
    t_deviceIndex device_index, uint8_t *p_afh_mode);

uint8_t LSLCafh_Count_1_Bits(const uint8_t bit_array[]);
 ...
 */
extern const uint8_t _LSLCafh_channel_bit_vector[AFH_BYTES_PER_CHANNEL_MAP+1];
extern const uint8_t _LMafh_bad_channel_bit_vector[AFH_BYTES_PER_CHANNEL_MAP];
