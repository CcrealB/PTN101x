...
extern void (*BTms_OS_Post)(uint32_t mask);
extern uint32_t (*BTms_OS_Pend)(uint32_t mask, uint16_t timout);
extern t_error (*BTms_Callback)(void);

void BTms_OS_Event_Dispatcher_Initialise( void (*os_post)(uint32_t),
             uint32_t (*os_pend)(uint32_t, uint16_t), uint16_t os_timeout);
...
RAM_CODE t_error BTms_Sched(uint32_t num_interations);
