...
#define to_u_int32(addr)   ((uint32_t)*(addr) + (*((addr)+1)<<8) + \
                           (*((addr)+2)<<16) + (*((addr)+3)<<24) )

#define to_u_int24(addr)   ((uint32_t)*(addr) + (*((addr)+1)<<8) + (*((addr)+2)<<16) )

#define to_u_int16(addr)   ((uint16_t)*(addr) + (*((addr)+1)<<8) )

__INLINE__ t_lap FHS_Get_LAP(const t_FHSpacket *fhs)
{
    const uint8_t *p_value32 = fhs->payload + 4;
...
{
    const uint8_t *p_value16 = fhs->payload + 9;
...
__INLINE__ void FHS_Set_LAP(t_FHSpacket *fhs, t_lap lap)
{
     ...
     */
    uint8_t *p_lap = fhs->payload + 4;

    *p_lap   = (uint8_t) _SYS_GET_CHAR_8_BITS((*p_lap & 0x03) | (uint8_t) (lap<<2));
    *++p_lap = (uint8_t) _SYS_GET_CHAR_8_BITS(lap>> 6);
    *++p_lap = (uint8_t) _SYS_GET_CHAR_8_BITS(lap>>14);
    ++p_lap;
    *p_lap   = (uint8_t) ((*p_lap & 0xFC) | ((uint8_t) (lap>>22) & 0x3));
}
...
{
    uint8_t *p_nap = fhs->payload + 9;
    *p_nap++ = (uint8_t) (nap & 0xFF);
    *p_nap   = (uint8_t) (nap >> 8);
}
...
{
    const uint8_t *p_value32 = fhs->payload + 14;
...
{
    return (uint8_t) ((fhs->payload[7] >> 2) & 0x1 );
}
#endif
...
{
    uint8_t *p_fhs_byte7 = fhs->payload + 7;
    *p_fhs_byte7 = (uint8_t) ((*p_fhs_byte7 & 0xFB) | (eir << 2 ));
}
#endif
...
{
    uint8_t *p_fhs_byte7 = fhs->payload + 7;
    *p_fhs_byte7 = (uint8_t) ((*p_fhs_byte7 & 0xCF) | (srMode << 4 ));
}

__INLINE__ void FHS_Set_Page_Scan_Period(t_FHSpacket *fhs, t_pageScanPeriod spMode)
{
    uint8_t *p_fhs_byte7 = fhs->payload + 7;
    *p_fhs_byte7 = (uint8_t) ((*p_fhs_byte7 & 0x3F) | (spMode << 6 ));
}

__INLINE__ void FHS_Set_Page_Scan_Mode(t_FHSpacket *fhs, t_pageScanMode smMode)
{
    uint8_t *p_fhs_byte17 = fhs->payload + 17;
    *p_fhs_byte17 = (uint8_t) ((*p_fhs_byte17 & 0x1F) | (smMode << 5 ));
}
...
{
    uint8_t *p_fhs_byte14 = fhs->payload + 14;
    *p_fhs_byte14 = (uint8_t) ((*p_fhs_byte14 & 0xF8) | (am_addr));
}
...
__INLINE__ void FHS_Get_Syncword_Ex(const t_FHSpacket *fhs, t_syncword *p_syncword)
{
    ...
    p_syncword->high |= (uint32_t)(((p_syncword->high)&BIT25) ? 0x4C000000L : 0xB0000000L);
}

__INLINE__ void FHS_Set_Syncword_Ex(t_FHSpacket *fhs, const t_syncword *p_syncword)
{
    uint8_t *p_fhs_payload = fhs->payload;
    uint8_t fhs_byte7 = fhs->payload[7];    /*Retain Page settings         */

    *p_fhs_payload++ = (uint8_t) _SYS_GET_CHAR_8_BITS(p_syncword->low);
    *p_fhs_payload++ = (uint8_t) _SYS_GET_CHAR_8_BITS(p_syncword->low>> 8);
    *p_fhs_payload++ = (uint8_t) _SYS_GET_CHAR_8_BITS(p_syncword->low>> 16);
    *p_fhs_payload++ = (uint8_t) (p_syncword->low>> 24);

    *p_fhs_payload++ = (uint8_t) _SYS_GET_CHAR_8_BITS(p_syncword->high);
    *p_fhs_payload++ = (uint8_t) _SYS_GET_CHAR_8_BITS(p_syncword->high >> 8);
    *p_fhs_payload++ = (uint8_t) _SYS_GET_CHAR_8_BITS(p_syncword->high >> 16);
    *p_fhs_payload++ = (uint8_t) (((p_syncword->high >> 24) & 0x3) |
                                 (fhs_byte7&0xFC));
...
__INLINE__ void FHS_Set_CLK(t_FHSpacket *fhs, t_clock clk)
{
     ...
     */
    uint8_t *p_fhs_clk = fhs->payload+14;
    clk >>= 2;
    *p_fhs_clk = (uint8_t) ((*p_fhs_clk & 0x07 ) | ((clk & 0x1F)<< 3));
    clk >>= 5;
    *++p_fhs_clk = (uint8_t) (clk & 0xFF);
    clk >>= 8;
    *++p_fhs_clk = (uint8_t) (clk & 0xFF);
    clk >>= 8;
    ++p_fhs_clk;
    *p_fhs_clk = (uint8_t) ((*p_fhs_clk & 0xE0) | (clk & 0x1F));
}
...
{
    uint8_t *p_fhs_device_class = fhs->payload+11;
    *p_fhs_device_class++ = (uint8_t) (class_of_device & 0xFF);
    class_of_device >>=8;
    *p_fhs_device_class++ = (uint8_t) (class_of_device & 0xFF);
    class_of_device >>=8;
    *p_fhs_device_class++ = (uint8_t) (class_of_device & 0xFF);
