...
void TCeg_Command_Status_Event(t_error status, uint16_t opcode);
void TCeg_Command_Complete_Event(uint16_t opcode, t_error status);
...
void TCeg_Send_LE_GATT_Event(uint8_t* le_gatt_data,uint8_t le_gatt_length, uint8_t tx0_rx1);
void TCeg_Send_LE_LLC_Event(const uint8_t* le_llc_data, uint8_t le_llc_length, uint8_t tx0_rx1);

#if (PRH_BS_CFG_SYS_TRACE_LE_DEVICE_STATE_VIA_HCI_SUPPORTED == 1)
void TCeg_Send_LE_LEDeviceState_Event(uint8_t state, t_LE_Connection* p_connection );
#endif

#if (PRH_BS_CFG_SYS_TRACE_LLDATA_VIA_HCI_SUPPORTED==1)
void TCeg_Send_LE_LLData_Event(uint16_t header, const uint8_t* le_ll_data, uint8_t le_lldata_length, uint8_t channel,uint8_t tx,uint8_t link_id);
#endif
/*
 * LMP Logger, used by lmp_cmd_disp.c/lmp_event_gen.c if activated
 */
#if (PRH_BS_CFG_SYS_TRACE_LMP_VIA_HCI_SUPPORTED==1)
void TCeg_Send_LMP_Event(const uint8_t* lmp_data, uint8_t lmp_length, uint8_t tx0_rx1);
#else
...
void TCeg_Send_LC_Event(t_ulc_procedure lc_event, t_state state, 
                        uint32_t lc_arg, uint8_t tx0_rx1);
#else
...
void TCeg_Send_BB_Event(uint8_t tx0_rx1);
#else
...
void TCeg_Send_Radio_Event(t_tci_eg_radio_state radio_state, uint32_t radio_arg);
...
void TCeg_Send_Interrupt_Event(t_tci_eg_interrupt interrupt_event, uint8_t interrupt_state);
#else
...
void TCeg_Send_LE_Adv_Event(uint16_t header, const uint8_t* le_adv_data, uint8_t le_adv_length, uint8_t channel,uint8_t tx);
#endif
...
void TCeg_Send_LE_SMP_Event(uint8_t* le_smp_data,uint8_t le_smp_length, uint8_t tx0_rx1);
#endif
#endif
