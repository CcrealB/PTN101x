...
void LEsecurity_Encrypt(t_p_pdu p_pdu, uint8_t *encrypted_data);
void LEsecurity_Rand(uint8_t *random_number_for_host);
t_error LEsecurity_Start_Encryption(uint16_t handle, uint8_t* p_rand,uint16_t encry_div,uint8_t* p_ltk );
...
void LEsecurity_HW_aes_encrypt_128(const uint8_t* plaintext_data, uint8_t* inv_encrypted_data,
								   const uint8_t* key,            uint8_t* output_key);		
...
t_error LEsecurity_GAP_Start_Encryption(t_LE_Connection* p_connection,uint8_t* random,uint16_t encry_div,uint8_t* ltk);
#endif
...
t_error LEsecurity_Write_Authenticated_Payload_Timeout(t_LE_Connection* p_connection,uint16_t timeout);
#endif
