 ...
 * Function prototypes
 */
t_error LEtest_Receiver_Test(uint8_t rx_freq);
t_error LEtest_Transmitter_Test(uint8_t tx_freq, uint8_t len_of_test_data,
								uint8_t packet_payload);
uint16_t LEtest_Test_End(void);
t_error LEtest_Ctrl_Handle_Test_Mode_Event(uint8_t IRQ_Type);
