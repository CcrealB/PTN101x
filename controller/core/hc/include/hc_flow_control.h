...
typedef struct s_hc_flow_crtl
{
    ...
    uint8_t   host_SYN_Data_Packet_Length;
    uint16_t  host_Total_Num_ACL_Data_Packets;
    uint16_t  host_Total_Num_SYN_Data_Packets;

    uint16_t  host_available_number_ACL_data_packets;
    uint16_t  host_available_number_SYN_data_packets;
    uint8_t   hc2host_flow_control_enable;

    /*
     * Host to Host Controller flow control parameters
     */
    uint8_t   host2hc_SYN_flow_control_Enable;
    t_timer  hc_completed_data_packets_report_timer;
    t_slots  hc_completed_data_packets_report_timeout;
    uint8_t   hc_completed_ACL_data_packets_report_threshold;
    uint8_t   hc_completed_SYN_data_packets_report_threshold;
...
uint8_t  HCfc_Read_SYN_Flow_Control_Enable(void);
t_error HCfc_Write_SYN_Flow_Control_Enable(uint8_t sco_flow_control_enable);
...
t_q_descr *HCfc_Get_Next_HC_to_Host_SYN_Descriptor(uint8_t sco_index);
...
t_error HCfc_Set_Host_Controller_To_Host_Flow_Control(uint8_t flow_control_enable);
uint8_t  HCfc_Read_Host_Controller_To_Host_Flow_Control(void);
...
t_error HCfc_Set_Host_To_Host_Controller_Flow_Control_Thresholds(
    uint8_t num_ACL_packets_threshold, uint8_t num_SYN_packets_threshold, 
    uint16_t timeout_threshold_slots);
