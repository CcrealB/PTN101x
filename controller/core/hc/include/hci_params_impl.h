 ...
 *
 *****************************************************************/
__INLINE__ uint16_t HCIparam_Get_Uint16(t_p_pdu p_pdu)
{
    return ((uint16_t)p_pdu[0] + ((uint16_t) p_pdu[1] << 8));
}
 ...
 *
 *   DESCRIPTION :
 *   retrieves a LAP (part of the BD_Address) from a buffer of uint8_t's
 *
 *****************************************************************/
 ...
 *
 *   DESCRIPTION :
 *   retrieves a BD_ADDRESS from a buffer of uint8_t's
 *
 *****************************************************************/
...
__INLINE__ uint16_t HCIparam_Get_Packet_Types(t_p_pdu p_pdu)
{
...
__INLINE__ uint32_t HCIparam_Get_Uint24(t_p_pdu p_pdu)
{
    return ( (uint32_t)(*p_pdu) + ((uint32_t)(*(p_pdu+1)) << 8) 
           + ((uint32_t)(*(p_pdu+2)) <<16 ));
}
...
__INLINE__ uint32_t HCIparam_Get_Uint32(t_p_pdu p_pdu)
{
    return ( (uint32_t)(*p_pdu) + ((uint32_t)(*(p_pdu+1)) << 8) + 
           ( (uint32_t)(*(p_pdu+2)) <<16 ) + ((uint32_t)(*(p_pdu+3)) <<24 ));
}
...
__INLINE__ uint8_t HCIparam_Get_Length(t_p_pdu pdu_ptr)
{
   return *pdu_ptr;
}
...
__INLINE__ uint8_t HCIparam_Get_Inquiry_Length(t_p_pdu p_pdu)
{
   return (uint8_t)*p_pdu;
}
...
__INLINE__ uint8_t HCIparam_Get_Num_Responses(t_p_pdu p_pdu)
{
   return (uint8_t)*p_pdu;
}
...
__INLINE__ uint16_t HCIparam_Get_HCI_Opcode(t_p_pdu p_pdu)
{
   return HCIparam_Get_Uint16(p_pdu);
}
 ...
 *   
 *****************************************************************/
__INLINE__ uint8_t HCIparam_Get_Allow_Role_Switch(t_p_pdu p_pdu)
{
    return (*p_pdu);
}
