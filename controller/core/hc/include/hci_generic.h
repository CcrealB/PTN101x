 ...
 *
 *              extern void* (*HC_MemAlloc)(uint32_t); // defaults = malloc
...
int HCI_Generic_Initialise( uint16_t num_in_acl_packets,
                            ...
                            uint32_t* flash_config_addr,
                            uint32_t hw_config_info);
 ...
 *
 * void HC_Event_Post(uint32_t mask)
 * {
 *     if(get_imask())
 ...
 * }
 *
 * uint32_t HC_Event_Pend(uint32_t mask, uint16_t timout)
 * {
 *     FLGPTN hc_flgptn = 0;
 *     twai_flg(HC_EVENT_FLAGS, mask, TWF_ORW, &hc_flgptn, timeout);
 *     return (uint32_t)hc_flgptn;
 * }
 *
...
void HCI_Generic_Register_Tx_Callback(void (*callback_function)(uint8_t *data, uint32_t pdu_len, uint8_t *header, 
                            uint8_t head_len));
