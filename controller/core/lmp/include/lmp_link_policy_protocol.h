...
t_error LMpolicy_LM_Write_Hold_Mode_Activity(t_lmp_link* p_link,uint8_t mode,t_cmd_complete_event* cmd_complete_info);

t_error LMpolicy_LM_Hold_Mode(t_lmp_link* p_link,uint16_t max_interval, uint16_t min_interval);

void    LMpolicy_LM_Role_Discovery(t_lmp_link* p_link,t_cmd_complete_event* cmd_complete_info);

t_error LMpolicy_LM_Exit_Park_Mode(t_lmp_link* p_link,uint8_t automatic);
t_error LMpolicy_LM_Park_Mode(t_lmp_link* p_link,uint16_t max_interval,uint16_t min_interval);

t_error LMpolicy_LM_Sniff_Mode(t_lmp_link* p_link,uint16_t max_interval,
...
t_error LMpolicy_LMP_Not_Accepted(t_lmp_link* p_link,uint16_t opcode, t_error reason);
t_error _LMpol_Execute_Unpark_Request(t_lmp_link*  p_link, uint8_t  automatic, t_role role);


#if (PRH_BS_CFG_SYS_LMP_SNIFF_SUPPORTED==1)
#if (PRH_BS_CFG_SYS_LMP_SNIFF_SUBRATING_SUPPORTED==1)
t_error LMpolicy_LM_Sniff_Subrating(t_lmp_link* p_link, uint16_t max_latency,
