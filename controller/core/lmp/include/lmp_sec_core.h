...
void LMsec_core_Array_Xor(uint8_t ARRAY_SIZE, uint8_t* result, 
    uint8_t* dest, uint8_t* src);
uint8_t LMsec_core_Auth_Allowed(t_bd_addr *address);
void LMsec_core_Auth_Challenge(t_lmp_link *p_link, uint8_t tid_role);
void LMsec_core_Auth_Complete(t_lmp_link *p_link, t_error status);
void LMsec_core_Auth_Response(t_lmp_link *p_link, uint8_t tid_role, 
    t_rand au_rand);
...
uint8_t LMsec_core_Get_Broadcast_Key_Length(void);
BOOL LMsec_core_Is_Broadcast_Key_Length_Possible(void);
