...
{   
...
#endif
    uint32_t beacon_instant;
    t_slots accept_timeout;
    uint8_t  accept_timeout_index;
    ...
    */
    ...
    t_pin_type pin_type;
    uint8_t max_encrypt_key_size;
    uint8_t min_encrypt_key_size;
     ...
     */
    uint8_t new_connections_allowed;
    uint8_t num_acl_links;
    uint8_t num_slave_acl_links;
    uint8_t num_master_acl_links;
    /*
     * SCO connection related
     */
    uint8_t num_slave_sco_links;
    uint8_t num_master_sco_links;

    /* 
     * The Broadcast Connection Handle 
     */
    uint16_t active_broadcast_handle;
    uint16_t piconet_broadcast_handle;

    uint8_t num_bcast_retry;
    uint8_t current_bcast_type;
    ...
    uint8_t  N_bcast;
    uint32_t end_beacon;
    uint32_t next_acc_window;  /* The instance of the next access window  */
    uint32_t start_acc_window;
    uint32_t end_acc_window;
    uint8_t T_access;
    uint8_t D_access;
    uint8_t N_access;
    uint8_t M_access;
    uint8_t M_access_rem; /* The number of Access Windows remaining    */
    uint8_t access_scheme;
    uint8_t N_poll_rem;    /* The number of Poll Slots remaining       */
    uint8_t N_poll;        
    uint8_t N_sleep;
    uint8_t D_sleep;
    uint16_t unpark_handle;    /* The handle for the ACL link currently being unparked */
    uint16_t bcast_scan_ext;   /* Broadcast Scan Window Extension */
	uint8_t bcast_scan_ext_unpark;
    uint8_t park_defaults;
#endif
    uint16_t default_t_poll;
    t_packet default_pkt_type;
    t_packet last_broadcast_pkt_rx;
    uint8_t links_in_low_power;
    t_slots next_wakeup_time;  
    t_lmp_link* next_wakeup_link;

#if (PRH_BS_CFG_SYS_SLEEP_IN_STANDBY_SUPPORTED == 1)
    uint8_t standby_power_timer_index;
#endif

#if (PRH_BS_CFG_SYS_LMP_POWER_CONTROL_SUPPORTED==1) 
    uint8_t peer_power_timer_index;
#endif

#if(PRH_BS_CFG_SYS_HCI_V12_FUNCTIONALITY_SUPPORTED==1)
    uint16_t default_link_policy_mode;
#endif

#if ((PRH_BS_CFG_SYS_LMP_SCO_SUPPORTED==1) || (PRH_BS_CFG_SYS_LMP_EXTENDED_SCO_SUPPORTED==1))
    uint16_t default_voice_setting;
#endif

#if(PRH_BS_CFG_SYS_BROADCAST_NULL_IN_INQ_PAGE_SUPPORTED==1)
    uint16_t emergency_poll_interval;
#endif
#if (PRH_BS_CFG_SYS_LMP_SECURE_SIMPLE_PAIRING_SUPPORTED==1)
	uint8_t Outgoing_Encapsulated_P192_len_pending;
	uint8_t public_key_x[24];
	uint8_t public_key_y[24];
...
#endif
#if (PRH_BS_CFG_SYS_SSP_OOB_SUPPORTED==1)
	uint8_t randomizer_R[16];
	uint8_t hash_C[16];
#endif
	//uint8_t Ra[16];
#endif
	uint8_t default_sco_route_via_hci;
	uint8_t inquiry_tx_power_level;
	uint8_t inquiry_response_tx_power_level;
#if 1 // FIX FOR SSP- Blocking
	uint8_t new_public_key_generation; // Key Generation Status
#endif
#if (PRH_BS_CFG_SYS_LE_CONTROLLER_SUPPORTED==1)
	uint8_t le_host_supported;
	uint8_t le_and_bredr_host;
#endif
	uint8_t SCO_blocking_scatternet;
    uint8_t crystal_calibration;
...
void LMconfig_LM_Change_Local_Name(uint8_t* p_name);
void LMconfig_LM_Write_Voice_Settings(uint16_t voice_settings);
void LMconfig_LM_Standby_Low_Power_Timeout(t_lmp_link* p_link);

t_error LMconfig_LMP_Not_Accepted(t_lmp_link* p_link, uint16_t opcode, t_error reason);
...
uint8_t LMconfig_PageInq_PreventingScan(void);
/*
 * LMP state functions
 */
__INLINE__ void LMconfig_LM_Clear_Device_State(uint32_t state);
__INLINE__ void LMconfig_LM_Set_Device_State(uint32_t state);
__INLINE__ uint32_t LMconfig_LM_Page_Scans_Allowed(void);
__INLINE__ uint32_t LMconfig_LM_Inquiry_Scans_Allowed(void);
__INLINE__ uint32_t LMconfig_LM_Periodic_Inquiry_Allowed(void);
__INLINE__ uint32_t LMconfig_LM_Check_Device_State(uint32_t query_state);
__INLINE__ uint32_t LMconfig_LM_Enforce_Scans_Disabled(void);

__INLINE__ BOOL LMconfig_LM_Connected_As_Master(void);
__INLINE__ BOOL LMconfig_LM_Connected_As_Slave(void);
__INLINE__ BOOL LMconfig_LM_Connected_As_Scatternet(void);
__INLINE__ uint8_t LMconfig_LM_Num_Piconets(void);
__INLINE__ BOOL LMconfig_LM_Connected_As_SCO_Master(void);
__INLINE__ BOOL LMconfig_LM_Connected_As_SCO_Slave(void);
#ifdef BT_DUALMODE

__INLINE__ BOOL LMconfig_LM_Connected_As_Classic_LE_Scatternet(void);
#endif
/*
 * System based
 */
