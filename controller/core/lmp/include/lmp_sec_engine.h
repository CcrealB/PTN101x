...
void LM_SecEng_Authenticate_E1(uint8_t* p_randinput, t_link_key key,
...
void LM_SecEng_Generate_Key_E22(const t_rand randinput, t_pin_code PIN,
        uint8_t lengthPIN, const t_bd_addr* p_bd_addr, t_link_key outputkey);
void LM_SecEng_Generate_Key_E3(t_rand encr_rand, t_cof cof,
        t_link_key current_link_key, t_rand encr_key);
void LM_SecEng_Generate_Kc_Prime(uint8_t * kc, uint8_t kc_prime_len,
        uint8_t *kc_prime);
