...
BOOL LMpol_Get_Next_Slave_Tx_Activity(t_deviceIndex device_index);
BOOL LMpol_Get_Next_Slave_Rx_Activity(t_deviceIndex device_index);

t_devicelink* LMpol_Get_Next_Master_Beacon_Tx(void);

BOOL LMpol_Get_Next_Slave_Park_Activity(t_lmp_link* p_link);

t_error LMpol_Max_Slots_Allowed(t_lmp_link* p_link, uint8_t max_slots);
t_error LMpol_Device_Activity(t_LC_Event_Info* lc_event_info);

t_error LMpolicy_Write_Flush_Timeout(t_lmp_link* p_link,uint16_t timeout);
...
void LMpol_Init_SCO(t_lmp_link* p_link,uint8_t sco_index);
...
#endif
void LMpol_Deactivate_SCO(t_lmp_link* p_link, uint8_t sco_index);
void LMpol_LM_SSR_Initiate(t_lmp_link* p_link);
void LMpol_LM_SSR_Timeout(t_lmp_link* p_link);
void LMpol_Init_Sniff_Subrating_Instant(t_lmp_link* p_link, uint32_t ssr_instant);
void LMpol_Sleep(uint16_t interval,t_lmp_link* p_link, uint32_t clock);
void LMpol_Decr_Links_In_Low_Power(void);
void LMpol_Adjust_Wakeup_Slot_Time(t_lmp_link *p_link, uint32_t instant);
...
uint32_t LMpol_Get_Common_Subrate(t_lmp_link* p_link);
...
void LMpol_Adjust_Poll_Position_On_All_Links(int16_t poll_position);

void LMpol_Adjust_ACL_Packet_Types_On_Link(t_lmp_link* p_link, t_LM_context context);
BOOL LMpol_Sniff_Activation_On_Next_Frame(void);
...
BOOL LMpol_Retransmission_Allowed(t_sco_info *p_sco_link,t_devicelink* pDL, t_clock  current_clk); 
#endif

t_slots LMpol_Check_Slots_To_Next_Classic_Activity(uint32_t current_clk);
t_slots LMpol_Check_Slots_To_Next_Classic_High_Priority_Activity(uint32_t current_clk);
#ifdef BT_DUALMODE

BOOL LMpol_BLE_Check_Need_Switch_To_Classic_Bt(void);
BOOL LMpol_BLE_Check_BT_Is_Scan(void);
#endif
#endif
