...
t_error LMconnection_LM_Read_Remote_Extended_Features(t_lmp_link* p_link, uint8_t page);
...
t_error LMconnection_LM_Add_SCO_Connection(t_lmp_link* p_link,
    t_packetTypes packet_types, uint16_t voice_setting);
...
t_error LMconnection_LM_Setup_Synchronous_Connection(uint16_t handle,
...
t_error LMconnection_LM_Accept_Synchronous_Connection_Request(uint8_t* p_bdaddr,
    uint32_t tx_bandwidth, uint32_t rx_bandwidth, uint16_t max_latency,
    uint16_t voice_setting, uint8_t retransmission_effort, 
    uint16_t esco_packet_types);
t_error LMconnection_LM_Reject_Synchronous_Connection_Request(uint8_t* p_bdaddr,
    t_error reason);
...
t_error LMconnection_LM_Accept_Synchronous_Connection_Request_Common(t_lmp_link* p_link,
    ...
    uint8_t retransmission_effort, uint16_t esco_packet_types);
...
t_error LMconnection_LMP_Accepted(t_lmp_link* p_link, uint16_t opcode);
t_error LMconnection_LMP_Not_Accepted(t_lmp_link* p_link,uint16_t opcode, t_error reason);
...
void LMch_Adjust_Next_Change_Piconet_Time(t_slots timeout, BOOL extend);
void LMch_Clear_Change_Piconet_Timer(void);

void LMch_Send_LMP_Packet_Type_Table_Req(t_lmp_link *p_link, uint8_t ptt);
void LMconnection_LMP_Remove_Sco_Link_Accepted_Ack(t_deviceIndex device_index);

RAM_CODE void LMch_Enable_L2CAP_Traffic(t_lmp_link* p_link, uint8_t l2cap_enable_type);
void LMch_Disable_L2CAP_Traffic(t_lmp_link* p_link, uint8_t l2cap_disable_type);

void LMconnection_Page_Resume(t_lmp_link* p_link);
void LMconnection_Set_Suspend_Timer(uint8_t suspend_time);
/*
