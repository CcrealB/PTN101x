...
/*****************************************************************************
 * FUNCTION: hw_memcpy128   [Copies 128 bits at a time for both uint16_t/32]
 *
...
__INLINE__ void * hw_memcpy128(void *dest, const void *src, size_t size)
{
    ...
    /*
     * During test ensure that addresses are actually uint32_t aligned.
	 * Note - align not necessary for 32 bit data on TL4.
     */
    assert( ((uint32_t)dest & sizeof(u_intHW)-1) == 0 && 
            ((uint32_t)src  & sizeof(u_intHW)-1) == 0); 
#endif
/******
#if (CONFIG_CPU_CLK_OPTIMIZATION == 1)
    bk3000_set_clk(3,1);
...
*****/
    ...
    u_intHW *dest_ptr = (u_intHW*) dest;
    uint32_t buffer128[4];
    uint8_t *_u8_src  = (uint8_t *) src;

    /*
     * Note the u_intHW pointer hence the offset is by u_intHW
     */
    uint8_t *src_past_end_ptr = _u8_src + size;
    ...
    while(_u8_src < src_past_end_ptr)
    {
        ...
        uint8_t *_u8_dest = (uint8_t *) buffer128;
        while (_u8_dest < (uint8_t *) buffer128 + sizeof(buffer128))
        {
            *_u8_dest++ = *_u8_src++;
        }
...
{
    uint32_t *dest_cpy = (uint32_t*) dest,
            *src_cpy = (uint32_t*) src;
    uint32_t *dest_end = (uint32_t*)((uint32_t)dest_cpy+(size/ADDR_DIV));
/**************************************
#if (CONFIG_CPU_CLK_OPTIMIZATION == 1)
    bk3000_set_clk(3,1);
#endif
***************************************/
	 ...
	 * Note - align not necessary for 32 bit data on TL4.
     */
    assert( ((uint32_t)dest & 0x03) == 0 && ((uint32_t)src & 0x03) == 0); 
#endif
...
***************************************/
	 ...
	 * Note - align not necessary for 32 bit data on TL4.
     */
	assert( (((uint32_t)dest & ((32/CHAR_BIT)-1)) == 0)
			&& (((uint32_t)src & ((32/CHAR_BIT)-1)) == 0) ); 
#endif
...
{
    uint8_t *_u8_dest = (uint8_t *) dest;
    uint8_t *_u8_end  = (uint8_t *) dest + size;
    uint8_t *_u8_src  = (uint8_t *) src;
/**************************************
#if (CONFIG_CPU_CLK_OPTIMIZATION == 1)
    bk3000_set_clk(3,1);
 ...
 *****************************************************************************/
__INLINE__ void hw_memset8(void * dest, uint8_t value, size_t size)
{
    uint8_t *_u8_dest = (uint8_t *)dest;
    uint8_t *_u8_end  = (uint8_t *)dest + size;
/**************************************
...
{
    uint32_t *p_src32 = (uint32_t *)src;
    uint8_t  *p_dest8   = (uint8_t  *)dest;
    uint32_t word32;
    uint8_t *p_dest8_end = p_dest8+size;
/**************************************
#if (CONFIG_CPU_CLK_OPTIMIZATION == 1)
    bk3000_set_clk(3,1);
#endif
...
{
    uint32_t *p_dest32 = (uint32_t *)dest;
    uint8_t  *p_src8   = (uint8_t  *)src;
    uint32_t word32;
    uint8_t *p_src8_end = p_src8+size;
/**************************************
#if (CONFIG_CPU_CLK_OPTIMIZATION == 1)
    bk3000_set_clk(3,1);
#endif
...
__INLINE__ void hw_memcpy_byte_pairs_from_word32(void *dest, const void *src, size_t size)
{
    ...
    while(p_dest8 < p_dest8_end)
    {
         ...
         */
		word32 = *p_src32++;
        *p_dest8++ = (uint8_t) word32;
        *p_dest8++ = (uint8_t) (word32>>8);
    }
}
#endif
