 ...
 * +----+----+----+----+----+----+----+----+
 *
 * Bit [0]  : Hide service visibility
 * Bit [1-7]: Reserved for future use
 */
enum attm_ext_svc_perm_mask
{
    /// Service visibility - Hide = 1
    PERM_MASK_EXT_SVC_HIDE = 0x01,
    PERM_POS_EXT_SVC_HIDE  = 0,
};
...
/**
 ****************************************************************************************
 * @brief Callback definition used to retrieve value of database hash
 *
 * @param[in] status   Execution status
 * @param[in] p_hash   Hash value computed
 * @param[in] cb_info  Source info used to retrieve requester
 ****************************************************************************************
 */
typedef void (*attmdb_hash_res_cb) (uint8_t status, const uint8_t* p_hash, uint32_t cb_info);
...
struct attm_svc_desc
{
    ...
    uint8_t  perm;
    /// Extended Service Permission (@see attm_ext_svc_perm_mask)
    uint8_t  ext_perm;
...
/// Structure use to compute data hash
struct attm_db_hash_input
{
    /// function that handle computation result
    attmdb_hash_res_cb        res_cb;
    /// value of the hash to compute
    uint8_t                   value[__ARRAY_EMPTY];
};
...
struct attm_db
{
    /// Hash input pointer used for hash computation
    struct attm_db_hash_input* p_hash_input;
    ...
    /// Used to know if database hash is valid or not
    bool    hash_valid;
...
/**
 ****************************************************************************************
 * @brief Compute Hash of the internal database.
 *
 *
 * @param[in] res_cb  Callback that will handle the result.
 * @param[in] cb_info Requester source information value returned in callback
 *
 * @return Command status code (if GAP_ERR_NO_ERROR, wait for callback execution)
 ****************************************************************************************
 */
uint8_t attmdb_hash_compute(attmdb_hash_res_cb res_cb, uint32_t cb_info);
