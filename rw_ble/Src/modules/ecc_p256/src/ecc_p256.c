 ...
 ****************************************************************************/
__INLINE void initBigNumber256(bigHex256 *BigHexResult);
__INLINE void setBigNumberLength256( bigHex256 *BigHexResult);
__INLINE void setVeryBigNumberLength256(veryBigHex256 *BigHexResult);
__INLINE void copyBigHex256(const bigHex256 *source,bigHex256 *destination);
__INLINE void shiftLeftOneArrayElement256(bigHex256 *input);
__INLINE void divideByTwo256(bigHex256* A);

__INLINE int isGreaterThan256(const bigHex256 *bigHexA,const bigHex256 *bigHexB);
__INLINE int isGreaterThanOrEqual256(const bigHex256 *bigHexA,const bigHex256 *bigHexB);
__INLINE int isVeryBigHexGreaterThanOrEqual256(const veryBigHex256 *bigHexA,const veryBigHex256 *bigHexB);

__INLINE void GF_Setup_Jacobian_Infinity_Point256(ECC_Jacobian_Point256 *infinity);
__INLINE void GF_Affine_To_Jacobian_Point_Copy256(const ECC_Point256 *source,ECC_Jacobian_Point256 *destination);
__INLINE void GF_Jacobian_Point_Copy256(const ECC_Jacobian_Point256 *source,ECC_Jacobian_Point256 *destination);
...
void specialModP256(bigHex256 *tmpHexA)
{
       ...
       (tmpHexA->sign == 1))
   {
       ...
       if (tmpHexA->len > bigHexP256.len)
       {
           // approach
            #if (ECC_MULT_ALGO_TYPE == 16)
           u_int16 SignificantU16 = GETMOSTSIGNIGICANTU16_256(tmpHexA);
           MultiplyByU16ModP256(SignificantU16,&NModP);
            #elif (ECC_MULT_ALGO_TYPE == 32)
           u_int32 SignificantU32 = GETMOSTSIGNIGICANTU32_256(tmpHexA);
...
/*__INLINE*/ int notEqual256(const bigHex256 *bigHexA,const bigHex256 *bigHexB)
{
    u_int32 i;
...
__INLINE int Is_Infinite256(const ECC_Jacobian_Point256* pPointP)
{
...
__INLINE void GF_Point_Copy256(const ECC_Point256 *source,ECC_Point256 *destination)
{
 ...
 * - Return Q
 ******************************************************************************************/
static void ecc_point_multiplication_win_256(struct ecc_elt_tag* ecc_elt)
{
...
static bool ecc_is_valid_point(bigHex256* X_coord, bigHex256* Y_coord)
{
    ...
    if (ecc_is_valid_point(&PublicKey256.x,&PublicKey256.y))
    {
...

        {
            if (ecc_elt->key_type == ECC_DHKEY_GENERATION)
            {
                ...
                ecc_elt->win_4_table  = (ECC_Jacobian_Point256*) ke_malloc((sizeof(ECC_Jacobian_Point256) * 15), KE_MEM_NON_RETENTION);
