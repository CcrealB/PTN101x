...
struct nvds_env_tag
{
    ...
    void (*erase)(uint32_t const                    address,
		    ...
		    /// Function to read the device Address being in the NVDS memory space
    void (*flash_read)(uint8_t *buffer, uint32_t address, uint32_t len);
    /// Function to write the device Address being in the NVDS memory space
    void (*flash_write)(uint8_t *buffer, uint32_t address, uint32_t len);
    /// Function to erase the entire NVDS memory space
    void (*flash_erase)(uint32_t address, uint8_t erase_size);
...
#endif
    /// env ble enable 
    NVDS_TAG_BLE_ENABLE                 = 0xAA,

    /// env ble adv_enable 
    NVDS_TAG_BLE_ADV_CONFIG_ENABLE      = 0xAB,

    /// env ble adv_enable 
    NVDS_TAG_BLE_UPDATE_CONFIG_ENABLE   = 0xAC,
