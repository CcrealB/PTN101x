...
/// Magic number offset
#define NVDS_MAGIC_NUMBER_ADDRESS              FLASH_ENVNVDS_DEF_ADDR_ABS
...
/// NVDS in RAM if no NVDS via SPI FLASH
 ...
 ****************************************************************************************
 */
static uint8_t nvds_ram_init(uint8_t *base, uint32_t len);
 ...
 ****************************************************************************************
 */
static void nvds_ram_read(uint32_t address, uint32_t length, uint8_t *buf);
 ...
 ****************************************************************************************
 */
static void nvds_ram_write(uint32_t address, uint32_t length, uint8_t *buf);
 ...
 ****************************************************************************************
 */
static void nvds_ram_erase(uint32_t address, uint32_t length);
...
static void nvds_read(uint32_t address, uint32_t length, uint8_t *buf)
{
    ...
    // Read the memory
    nvds_env.flash_read(buf, address, length);
}
...
static void nvds_write(uint32_t address, uint32_t length, uint8_t *buf)
{
    ...
    // Read the memory
    nvds_env.flash_write(buf, address, length);
}

static void nvds_erase(uint32_t address, uint32_t length)
{
    // Erase NVDS
    nvds_env.flash_erase(address, length);
}
#endif // (NVDS_READ_WRITE == 1)
