...
enum findl_msg_id
{
    ...
    FINDL_ENABLE_REQ = TASK_FIRST_MSG(TASK_ID_FINDL),//!< FINDL_ENABLE_REQ
    /// Enable confirm message, containing IAS attribute details if discovery connection type
    FINDL_ENABLE_RSP,                                //!< FINDL_ENABLE_RSP

    /// Alert level set request
    FINDL_SET_ALERT_REQ,                             //!< FINDL_SET_ALERT_REQ
    /// Alert level set response
    FINDL_SET_ALERT_RSP,                             //!< FINDL_SET_ALERT_RSP
};
