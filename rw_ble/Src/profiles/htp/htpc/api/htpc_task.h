...
enum htpc_msg_id
{
    ...
    HTPC_ENABLE_REQ = TASK_FIRST_MSG(TASK_ID_HTPC),//!< HTPC_ENABLE_REQ
    /// Confirm that cfg connection has finished with discovery results, or that normal cnx started
    HTPC_ENABLE_RSP,                               //!< HTPC_ENABLE_RSP


    /// Write Health Thermometer Notification Configuration Value request
    HTPC_HEALTH_TEMP_NTF_CFG_REQ,                  //!< HTPC_HEALTH_TEMP_NTF_CFG_REQ
    /// Write Health Thermometer Notification Configuration Value response
    HTPC_HEALTH_TEMP_NTF_CFG_RSP,                  //!< HTPC_HEALTH_TEMP_NTF_CFG_RSP

    ///APP request for measurement interval write
    HTPC_WR_MEAS_INTV_REQ,                         //!< HTPC_WR_MEAS_INTV_REQ
    ///APP request for measurement interval write
    HTPC_WR_MEAS_INTV_RSP,                         //!< HTPC_WR_MEAS_INTV_RSP

    /// Temperature value received from peer sensor
    HTPC_TEMP_IND,                                 //!< HTPC_TEMP_IND
    /// Measurement interval update indication received from peer sensor
    HTPC_MEAS_INTV_IND,                            //!< HTPC_MEAS_INTV_IND

    /// Generic message to read a HTP characteristic value
    HTPC_RD_CHAR_REQ,                              //!< HTPC_RD_CHAR_REQ
    /// Read HTP characteristic value response
    HTPC_RD_CHAR_RSP,                              //!< HTPC_RD_CHAR_RSP
};
