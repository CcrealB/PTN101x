...
enum cscps_msg_id
{
    ...
    CSCPS_ENABLE_REQ = TASK_FIRST_MSG(TASK_ID_CSCPS),//!< CSCPS_ENABLE_REQ
    /// Enable the CSCP Sensor task for a connection
    CSCPS_ENABLE_RSP,                                //!< CSCPS_ENABLE_RSP

    /// Send a CSC Measurement to the peer device (Notification)
    CSCPS_NTF_CSC_MEAS_REQ,                          //!< CSCPS_NTF_CSC_MEAS_REQ
    /// Inform that CSC Measurement sent to peer devices.
    CSCPS_NTF_CSC_MEAS_RSP,                          //!< CSCPS_NTF_CSC_MEAS_RSP

    /// Indicate that the SC Control Point characteristic value has been written
    CSCPS_SC_CTNL_PT_REQ_IND,                        //!< CSCPS_SC_CTNL_PT_REQ_IND
    /// Application response after receiving a CSCPS_SC_CTNL_PT_REQ_IND message
    CSCPS_SC_CTNL_PT_CFM,                            //!< CSCPS_SC_CTNL_PT_CFM

    /// Indicate a new NTF/IND configuration to the application
    CSCPS_CFG_NTFIND_IND,                            //!< CSCPS_CFG_NTFIND_IND

    /// Send a complete event status to the application
    CSCPS_CMP_EVT,                                   //!< CSCPS_CMP_EVT
};
