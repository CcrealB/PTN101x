                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    bk_printf("ancsc_enable_req_handler\r\n");
                                     ...
                                     ke_task_id_t const src_id)
{
    ...
    if(state == ANCSC_BUSY)
    {
		...
		bk_printf("ANCSC_NTF_SOURCE_CHAR  char handle = 0x%x,value handle = 0x%x,prop = 0x%x\r\n",ancsc_env->env[conidx]->ancsc.chars[ANCSC_NTF_SOURCE_CHAR].char_hdl,
		...
		bk_printf("ANCSC_DATA_SOURCE_CHAR  char handle = 0x%x,value handle = 0x%x,prop = 0x%x\r\n",ancsc_env->env[conidx]->ancsc.chars[ANCSC_DATA_SOURCE_CHAR].char_hdl,
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    if(state == ANCSC_IDLE)
    {
        ...
        else
        {
            ...
            {
                status = GAP_ERR_NO_ERROR;
				//bk_printf("data desc handle = 0x%x\r\n",handle);
                // Send GATT Write Request
                prf_gatt_write_ntf_ind(&ancsc_env->prf_env, conidx, handle, param->ntf_cfg,0);
                // store context of request and go into busy state
                ancsc_env->env[conidx]->operation = ke_param2msg(param);
    ...
    {
		//bk_printf("write data desc cfg busy !!!!\r\n");
        status = GAP_ERR_NO_ERROR;
        msg_status = KE_MSG_SAVED;
    }
                                         ...
                                         ke_task_id_t const src_id)
{
	...
	//bk_printf("cntlpoint handle = 0x%x\r\n",handle);
																					 ...
																					 
{
	...
	{
		//bk_printf("ancsc data source coming\r\n");
