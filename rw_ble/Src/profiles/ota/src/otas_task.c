                               ...
                               ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    if(ke_state_get(dest_id) == OTAS_IDLE)
    {
        ...
        if(cmd == NULL)
        {
            while(1) bk_printf("KE_MSG_ALLOC_DYN GATTC_SEND_EVT_CMD  == NULL \r\n");
        }
    ...
    {
        bk_printf("ota_tx_handler state busy!!!!\r\n");
    }
                                                ...
                                                ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    {	
		...
		if(otas_env == NULL)
		{
			while(1){ bk_printf("otas_env == null\r\n");};
		}
...
void otas_task_init(struct ke_task_desc *task_desc)
{
    ...
    struct otas_env_tag *srv_env = PRF_ENV_GET(OTAS, otas);
    ...
    task_desc->idx_max         = OTAS_IDX_MAX;
}
