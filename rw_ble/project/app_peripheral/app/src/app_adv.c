 ...
 * x05\xF0\x52\x57\x2D\x42\x4C\x45 - "BEKEN-BLE"
 * --------------------------------------------------------------------------------------
 */
#define APP_SCNRSP_DATA         "\x09\xFF\x05\xF0\x52\x57\x2D\x42\x4C\x45"
...
key_str_t adv_fsm_state_str[APP_ADV_STATE_NUM]=
{
    /* No Operation (if nothing has been requested)     */
    /* ************************************************ */
    /// No operation.
     {0x0,"APP_ADV_STATE_IDLE"},
     {0x1,"APP_ADV_STATE_CREATING"},
     {0x2,"APP_ADV_STATE_SETTING_ADV_DATA"},
     {0x3,"APP_ADV_STATE_SETTING_SCAN_RSP_DATA"},
     {0x4,"APP_ADV_STATE_UPDATA_ADV_DATA"},
     {0x5,"APP_ADV_STATE_CREATED"},
     {0x6,"APP_ADV_STATE_STARTING"},
     {0x7,"APP_ADV_STATE_STARTED"},
     {0x8,"APP_ADV_STATE_STOPPING"},

};


uint8_t *adv_fsm_key2str(uint8_t value)
{
    
    for(int i = 0;i < APP_ADV_STATE_NUM;i++)
    {
        if(adv_fsm_state_str[i].key == value)
        {
            return adv_fsm_state_str[i].str;
        }
    }
    return unknow_str;
}
...
//            memcpy(&cmd->info.direct, &peer_bd_addr, NVDS_LEN_PEER_BD_ADDRESS);
        ...
        uint16_t env_adv_intv = 0;
        if (nvds_get(NVDS_TAG_BLE_ADV_CONFIG_ENABLE, &len, (uint8_t *)&env_adv_intv) == NVDS_OK)
        {
            p_cmd->adv_param.prim_cfg.adv_intv_min = env_adv_intv*2;
            p_cmd->adv_param.prim_cfg.adv_intv_max = env_adv_intv*2;
        }
        else
        #endif //(NVDS_SUPPORT)
        {
        ...
        #if (BT_DUALMODE_RW == 1)
        // RWIP wakeup by external SW interrupt when deep sleeping, yangyang, 2021/03/10
...
void appm_set_adv_data(void)
{
    ...
    bk_printf("rwble adv %d:", p_cmd->length);
...
{
    //bk_printf("%s.adv_state(%s)\r\n", __func__, adv_fsm_key2str(app_env.adv_state));
    ...
    //bk_printf("End %s.adv_state(%s)\r\n", __func__, adv_fsm_key2str(app_env.adv_state));
}
#endif
uint8_t appm_update_adv_data(uint16_t length, uint8_t *p_data)
{
    // Returned status
