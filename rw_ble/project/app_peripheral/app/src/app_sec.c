                                 ...
                                 ke_task_id_t const src_id)
{
    ...
    app_sec_env.bond_info = param->info;
    switch (param->info)
    {
        ...
        {
            bk_printf("PAIRING_FAILED, err:0x%x\r\n", param->data.reason);
...
void app_sec_bond_cmd_req(uint8_t conidx)
{
	  struct gapc_bond_cmd *cmd = KE_MSG_ALLOC(GAPC_BOND_CMD,
            			KE_BUILD_ID(TASK_GAPC, conidx), KE_BUILD_ID(TASK_APP, conidx),//TASK_APP,//KE_BUILD_ID(TASK_APP, app_env.conidx),
            			gapc_bond_cmd);
	...
	cmd->pairing.auth = GAP_AUTH_BOND;
    cmd->pairing.key_size = 16;
    cmd->pairing.ikey_dist = GAP_KDIST_IDKEY;
    cmd->pairing.rkey_dist = GAP_KDIST_IDKEY | GAP_KDIST_ENCKEY;
...
void app_sec_encry_cmd_req(void)
{
	...
	bk_printf("send encryped start\r\n");
	extern uint8_t ltk_buffer[36];
    struct gapc_ltk ltk;
    // Length
    uint8_t length = NVDS_LEN_LTK;
	if (nvds_get(NVDS_TAG_LTK+app_env.conidx, &length,(uint8_t *)&ltk) != NVDS_OK)
    {
        ASSERT_ERR(0);
    }
    memcpy(&ltk,&ltk_buffer[6],NVDS_LEN_LTK);
#if 1
	bk_printf("data.ltk.ediv = 0x%x\r\n", ltk.ediv);
	bk_printf("data.ltk.ltk.key:");
    for (uint8_t i = 0; i < RAND_NB_LEN; i++)
    {
		bk_printf("0x%x ", ltk.ltk.key[i]);
    }
	bk_printf("\r\n");
	bk_printf("data.ltk.randnb.nb:");
	for (uint8_t i = 0; i < RAND_NB_LEN; i++)
    {
		bk_printf("0x%x ", ltk.randnb.nb[i]);
    }
	bk_printf("\r\n");
#endif
