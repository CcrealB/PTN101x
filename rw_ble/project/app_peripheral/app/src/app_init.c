...
/// Conn interval 
#define APP_CONN_INTV            (24) // *1.25ms, 30ms
//#define APP_CONN_INTV          (6)  // *1.25ms, 7.5ms
/// Conn window 
#define APP_CONN_LATENCY         (0)
#define APP_CONN_SUP_TO          (600) // *10ms, 6s
...
key_str_t init_fsm_state_str[APP_INIT_STATE_NUM]=
{
    /* No Operation (if nothing has been requested)     */
    /* ************************************************ */
    /// No operation.
     {0x0,"APP_INIT_STATE_IDLE"},
     {0x1,"APP_INIT_STATE_CREATING"},
     {0x2,"APP_INIT_STATE_CREATED"},
     {0x3,"APP_INIT_STATE_STARTING"},
     {0x4,"APP_INIT_STATE_STARTED"},
     {0x5,"APP_INIT_STATE_STOPPING"},
};
	
uint8_t *init_fsm_key2str(uint8_t value)
{
	
	for(int i = 0;i < APP_INIT_STATE_NUM;i++)
	{
		if(init_fsm_state_str[i].key == value)
		{
			return init_fsm_state_str[i].str;
		}
	}
	return unknow_str;
}
...
{
    bk_printf("%s.init_state(%s)\r\n",__func__,init_fsm_key2str(app_env.init_state));
    ...
    if (app_env.init_state == APP_INIT_STATE_CREATED)
    {
        ...
        p_cmd->operation = GAPM_START_INIT_ACTIVITY;
        ...
        p_cmd->u_param.init_param.peer_addr.addr_type = bdaddr.addr_type; //0;
      //  {{0x0f, 0x43, 0x45, 0x67, 0x89, 0xAB}}
        ...
        // And the next expected operation code for the command completed event
        app_env.init_op = GAPM_START_INIT_ACTIVITY;
    }
    return ret;
}
...
void appm_stop_connencting(void)
{
    ...
    // Fill the allocated kernel message
    cmd->operation = GAPM_STOP_INIT_ACTIVITY;
    ...
    // And the next expected operation code for the command completed event
    app_env.adv_op = GAPM_STOP_INIT_ACTIVITY;
}
...
{
    bk_printf("%s,state = %x,",__func__,ke_state_get(KE_BUILD_ID(TASK_APP,conidx)));
